> create at:2020年04月 4日  9:56:46 sha256:5dcde5292a9e01f61ba2e19945e25fe118726e940e8103364aade8894ff13521 *11549
# 摘 要
...
用户可以创建书单，并添加图书到自己所创建的书单中

|                    | 书单                                    |
| ------------------ | --------------------------------------- |
| 功能描述           | 对书单进行修改                          |
| 访问的数据库表     | 书单表 booklist                         |
| 进行的操作         | 增加 ，修改 ，删除书单 ，增加图书到书单 |
| 产生的结果         | 操作成功, 书单信息被需改                |
| 结果存储位置或输出 | 结果存储在书单表 booklist中             |
...
用户表(user)（用户ID，账号，密码，头像，姓名，性别，生日，是否被禁言 ，创建时间）

图书表(book) (图书 Id 图书isbn，图书名，单价，作者，出版社，出版时间，类别，评分，评分人数 收藏人数 喜爱人数 ，创建时间)

评论表(comment) (评论ID,评论PID,图书isbn,用户Id 评论内容 ，创建时间)

收藏表(savemap) (收藏ID 图书Id ,用户Id  是否删除 创建时间)

喜爱表(lovemap )(喜爱ID 图书Id ,用户Id  是否删除 创建时间)

书单表(booklist) (书单Id  书单编号   图书isbn 是否删除 创建时间)
